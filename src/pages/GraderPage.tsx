...
export function GraderPage({
  ...
  const currentProblem: ProblemItem | null =
    sortedProblems[currentIndex] ?? null

  const committedAnswer =
    currentProblem === null ? null : (responses[currentProblem.number] ?? null)
  ...
  const answeredCount = sortedProblems.filter((problem) =>
    hasAnswered(problem.number),
  ).length
    ...
    <div className="flex min-h-[80vh] flex-col">
      ...
      <div className="flex flex-1 flex-col justify-center py-8">
        <div className="rounded-lg border border-slate-200 bg-white py-16 text-center">
          ...
          <p className="mt-4 text-7xl font-bold text-black">
        ...
        <div className="space-y-3 pb-4">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onKeyDown={(event) => {
                    if (
                      event.key === 'Enter' &&
                      shortAnswerDraft.trim().length > 0
                    ) {
          ...
          <div className="flex gap-2">
            <button
              ...
              onClick={() =>
                onCurrentIndexChange(Math.max(currentIndex - 1, 0))
              }
            >
              <svg
                className="inline-block mr-2 -ml-1 h-4 w-4"
